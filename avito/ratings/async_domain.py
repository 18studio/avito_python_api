"""Async-доменные объекты пакета ratings."""
...
class AsyncReview(AsyncDomainObject):
...
@dataclass(slots=True, frozen=True)
class AsyncReviewAnswer(AsyncDomainObject):
...
@dataclass(slots=True, frozen=True)
class AsyncRatingProfile(AsyncDomainObject):
    ...
    async def get(
        ...
        return await self._execute(GET_RATINGS_INFO, timeout=timeout, retry=retry)


__all__ = ("AsyncRatingProfile", "AsyncReview", "AsyncReviewAnswer")
