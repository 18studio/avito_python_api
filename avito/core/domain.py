...
class AsyncDomainObject:
    """Базовый async-доменный объект с доступом к transport-слою."""
    ...
    async def _execute(
        ...
        *,
        ...
        """Выполняет v2 operation spec через общий async executor."""

        return await AsyncOperationExecutor(self.transport).execute(
            spec,
            path_params=path_params,
            query=query,
            request=request,
...
__all__ = ("AsyncDomainObject", "DomainObject")
